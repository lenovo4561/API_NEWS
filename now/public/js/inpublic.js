...
const Utils = {
    ...
    return `
      <div class="article-card" data-id="${articleId}">
        ...
        <div class="article-content">
          ...
          <div class="article-time">${this.escapeHtml(timeStr)}</div>
        </div>
      </div>
    `;
  },
  ...
  bindArticleCardEvents(detailPagePath = null) {
    ...
    const path =
      detailPagePath ||
      (isInPagesDir ? PATHS.detailPageFromPages : PATHS.detailPage);
...
class SidebarManager {
  ...
  renderSidebar() {
    ...
    const categoriesHTML = this.categories
      .map((categoryName) =>
        this.createCategoryItemHTML(categoryName, categoryBasePath)
      )
      .join("");
