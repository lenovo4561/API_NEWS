...
class ArticleDetailPage {
  ...
  async init() {
    const articlesPromise = this.prepareAllowedArticles();

    let optimisticLoadPromise = null;
    if (this.hasInitialValidId) {
      optimisticLoadPromise = this.loadArticleDetail();
    }

    await articlesPromise;

    let isRandomlySelected = false;
    let needNewLoad = false;
    ...
    if (needNewLoad || !optimisticLoadPromise) {
      await this.loadArticleDetail();
    } else if (optimisticLoadPromise) {
      await optimisticLoadPromise;
    }
  }
  ...
  renderArticle(article) {
    ...
    const articleType = article.type
      ? UnicodeDecoder.decode(article.type)
      : "-";
