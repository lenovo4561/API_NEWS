 ...
 * 检查文章是否已存在（通过标题）
 */
async function checkArticleExistsByTitle(title) {
  if (!title) return null;

  const sql = "SELECT id FROM info WHERE title = ? LIMIT 1";
  const result = await queryOne(sql, [title]);
...
async function saveArticlesToDB(articles, options) {
  ...
  for (const article of articles) {
    try {
      ...
      // 使用 INSERT IGNORE 语法，遇到重复标题自动跳过
      // 如果 title 有唯一索引，重复的记录会被忽略而不是报错
      const insertSql = `
      ...
      if (result.affectedRows > 0) {
        ...
        console.log(`⊘ 跳过重复: ${data.title.substring(0, 50)}...`);
        skipped++;
      }
    } catch (error) {
      console.error(
...
async function main() {
  ...
  try {
    ...
    console.log("  完成统计");
    console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    console.log(`  ✓ 新增保存: ${result.success} 篇`);
