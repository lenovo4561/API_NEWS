...
function getAllThirdLevelUris(node, uris = [], level = 0) {
  ...
  if (node.children && node.children.length > 0) {
    ...
    // 叶子节点（三级分类），并且不是根节点
    if (node.uri && node.uri !== "dmoz/Business") {
...
// 找到 Business 分类
const businessCategory = categoriesData.results.find(
  (cat) => cat.uri === "dmoz/Business"
);

if (!businessCategory) {
  console.error("❌ 未找到 Business 分类");
  process.exit(1);
}

// 获取所有 Business 三级分类的 URI
const allUris = getAllThirdLevelUris(businessCategory);

console.log(`✓ 找到 ${allUris.length} 个 Business 三级分类的 URI`);
console.log("开始遍历请求...\n");
