...
function getAllThirdLevelUris(node, uris = [], level = 0) {
  ...
  if (node.children && node.children.length > 0) {
    ...
    // 叶子节点（三级分类），并且不是根节点
    if (node.uri && node.uri !== "dmoz/Games") {
...
// 找到 Games 分类
const gamesCategory = categoriesData.results.find(
  (cat) => cat.uri === "dmoz/Games"
);

if (!gamesCategory) {
  console.error("❌ 未找到 Games 分类");
  process.exit(1);
}

// 获取所有 Games 三级分类的 URI
const allUris = getAllThirdLevelUris(gamesCategory);

console.log(`✓ 找到 ${allUris.length} 个 Games 三级分类的 URI`);
console.log("开始遍历请求...\n");
