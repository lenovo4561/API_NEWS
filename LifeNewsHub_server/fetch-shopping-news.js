...
function getAllThirdLevelUris(node, uris = [], level = 0) {
  ...
  if (node.children && node.children.length > 0) {
    ...
    // 叶子节点（三级分类），并且不是根节点
    if (node.uri && node.uri !== "dmoz/Shopping") {
...
// 找到 Shopping 分类
const shoppingCategory = categoriesData.results.find(
  (cat) => cat.uri === "dmoz/Shopping"
);

if (!shoppingCategory) {
  console.error("❌ 未找到 Shopping 分类");
  process.exit(1);
}

// 获取所有 Shopping 三级分类的 URI
const allUris = getAllThirdLevelUris(shoppingCategory);

console.log(`✓ 找到 ${allUris.length} 个 Shopping 三级分类的 URI`);
console.log("开始遍历请求...\n");
