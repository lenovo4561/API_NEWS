...
function getAllThirdLevelUris(node, uris = [], level = 0) {
  ...
  if (node.children && node.children.length > 0) {
    ...
    // 叶子节点（三级分类），并且不是根节点
    if (node.uri && node.uri !== "dmoz/Health") {
...
// 找到 Health 分类
const healthCategory = categoriesData.results.find(
  (cat) => cat.uri === "dmoz/Health"
);

if (!healthCategory) {
  console.error("❌ 未找到 Health 分类");
  process.exit(1);
}

// 获取所有 Health 三级分类的 URI
const allUris = getAllThirdLevelUris(healthCategory);

console.log(`✓ 找到 ${allUris.length} 个 Health 三级分类的 URI`);
console.log("开始遍历请求...\n");
