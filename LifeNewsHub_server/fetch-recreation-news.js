...
function getAllThirdLevelUris(node, uris = [], level = 0) {
  ...
  if (node.children && node.children.length > 0) {
    ...
    // 叶子节点（三级分类），并且不是根节点
    if (node.uri && node.uri !== "dmoz/Recreation") {
...
// 找到 Recreation 分类
const recreationCategory = categoriesData.results.find(
  (cat) => cat.uri === "dmoz/Recreation"
);

if (!recreationCategory) {
  console.error("❌ 未找到 Recreation 分类");
  process.exit(1);
}

// 获取所有 Recreation 三级分类的 URI
const allUris = getAllThirdLevelUris(recreationCategory);

console.log(`✓ 找到 ${allUris.length} 个 Recreation 三级分类的 URI`);
console.log("开始遍历请求...\n");
