...
function getAllThirdLevelUris(node, uris = [], level = 0) {
  ...
  if (node.children && node.children.length > 0) {
    ...
    // 叶子节点（三级分类），并且不是根节点
    if (node.uri && node.uri !== "dmoz/Sports") {
...
// 找到 Sports 分类
const sportsCategory = categoriesData.results.find(
  (cat) => cat.uri === "dmoz/Sports"
);

if (!sportsCategory) {
  console.error("❌ 未找到 Sports 分类");
  process.exit(1);
}

// 获取所有 Sports 三级分类的 URI
const allUris = getAllThirdLevelUris(sportsCategory);

console.log(`✓ 找到 ${allUris.length} 个 Sports 三级分类的 URI`);
console.log("开始遍历请求...\n");
